 ...
 * 
 * Fonctionnalités :
 * - Sélection d'un EC (Élément Constitutif) et d'une session d'examen
 * - Saisie de notes CC (Contrôle Continu - 40%) et Examen (60%)
 * - Calcul automatique de la note finale pondérée
 * - Gestion des absences
 * - Statistiques en temps réel (moyenne, taux de réussite, min/max, absents)
 * - Validation individuelle des notes
 * 
 * Règles de gestion :
 * - Note finale = (CC × 0.4) + (Examen × 0.6)
 * - Notes entre 0 et 20
 * - Absent : note finale = 0
 * - Réussite : note finale ≥ 10/20
 * 
...
/**
 * Composant principal de la page de saisie des notes
 * 
 * @returns Page de saisie des notes pour enseignants
 */
export default function TeacherGradesPage() {
  const qc = useQueryClient()
  
  // États locaux pour la sélection et la saisie
  ...
  // Mutation pour enregistrer une note avec validation
  const saveGradeMut = useMutation({
    mutationFn: (entry: GradeEntry) => {
      // Validation côté client
      const cc = parseFloat(entry.cc_grade)
      const exam = parseFloat(entry.exam_grade)
      
      if (entry.cc_grade && (cc < 0 || cc > 20)) {
        throw new Error('La note CC doit être entre 0 et 20')
      }
      if (entry.exam_grade && (exam < 0 || exam > 20)) {
        throw new Error("La note d'examen doit être entre 0 et 20")
      }
      
      return api.post('/evaluation/teacher/enter-grade/', {
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['teacher-grades', selectedEc, selectedSession] })
      qc.invalidateQueries({ queryKey: ['class-stats', selectedEc, selectedSession] })
    },
    onError: (error: any) => {
   ...
   * @param field - Champ à mettre à jour (cc_grade, exam_grade, is_absent)
