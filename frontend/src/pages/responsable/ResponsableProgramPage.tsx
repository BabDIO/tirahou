  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">Vue d'ensemble des programmes et performances académiques</p>
      </div>
            ...
            {!programs?.results?.length ? (
              ...
              <div className="space-y-2">
                {programs.results.map((p: { id: string; code: string; name: string; type_display: string; mode_display: string; status: string; capacity: number }) => (
                  <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-xl cursor-pointer hover:bg-primary-50 transition-colors"
                    onClick={() => { setSelectedProg(p.id); setTab('maquette') }}>
                    <div>
                      <p className="font-semibold text-sm text-gray-900 dark:text-gray-50">{p.name}</p>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500">{p.capacity} places</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {teachers.results.map((t: { id: string; user: { full_name: string; email: string }; grade_display: string; status: string; department_name: string | null }) => (
                  <div key={t.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 dark:text-gray-50 truncate">{t.user.full_name}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500">{t.grade_display} · {t.department_name ?? '—'}</p>
                    </div>
      ...
      {tab === 'maquette' && (
        <div className="space-y-4">
          <div>
            ...
            <select className="input bg-white dark:bg-slate-900 max-w-sm" value={selectedProg ?? ''}
                      ...
                      <div key={ue.id} className="px-5 py-3">
                        ...
                        {ue.ecs?.length > 0 && (
                          <div className="ml-4 space-y-1">
                            {ue.ecs.map(ec => (
                              <div key={ec.id} className="flex justify-between text-xs bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
                                <span><span className="font-mono font-semibold text-gray-700 dark:text-gray-300">{ec.code}</span> — {ec.name}</span>
                                <span className="text-gray-400 dark:text-gray-500">{ec.activity_type_display} · {ec.volume_hours}h</span>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                <tbody>
                  {results.results.map((r: { id: string; student: string; semester_label: string; average: number | null; credits_obtained: number; decision_display: string; decision: string; published: boolean }) => (
                    <tr key={r.id}>
                      ...
                      <td className="text-sm text-gray-600 dark:text-gray-400">{r.semester_label}</td>
                      <td>
