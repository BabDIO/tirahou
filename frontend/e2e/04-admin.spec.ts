...
test.describe('🔐 Administrateur', () => {
  ...
  test('devrait accéder au journal d\'audit', async ({ page }) => {
    await navigateTo(page, "Journal d'audit");

    await page.waitForTimeout(1500);

    await page.screenshot({ path: 'playwright-report/screenshots/admin-audit.png', fullPage: true });
  });

  test('devrait accéder à la gestion des parents & tuteurs', async ({ page }) => {
    await navigateTo(page, 'Parents & Tuteurs');

    await page.waitForTimeout(1500);

    await page.screenshot({ path: 'playwright-report/screenshots/admin-parents.png', fullPage: true });
  });

  test('devrait accéder aux badges & récompenses (gamification)', async ({ page }) => {
    await navigateTo(page, 'Badges & Récompenses');

    await page.waitForTimeout(1500);

    await page.screenshot({ path: 'playwright-report/screenshots/admin-gamification.png', fullPage: true });
  });
